...
const execute = async (wallet, backupTxs, coin, toAddress, feeRate) => {
  let coinBackupTx = backupTxs.filter(
    (b) => b.statechain_id === coin.statechain_id
  );

  if (coinBackupTx.length === 0) {
    throw new Error(
      `There is no backup transaction for the statechain id ${coin.statechain_id}`
    );
  }
  ...
  if (coinBackupTxs.length === 0) {
    throw new Error(
      `There is no backup transaction for the statechain id ${statechainId}`
    );
  }
  ...
  if (coin.status != CoinStatus.CONFIRMED) {
    throw new Error(
      `Coin status must be CONFIRMED to withdraw it. The current status is ${coin.status}`
    );
  }
  ...
  let txid = await electrumRequest({
