...
import { infoConfig } from "../wallet/utils.js";
...
const execute = async (wallets) => {
  const serverInfo = await infoConfig();
  ...
  for (const wallet of wallets) {
    for (let coin of wallet.coins) {
      ...
      let encMessages = await getMsgAddr(coin.auth_pubkey);
...
const process_encrypted_message = async (
  coin,
  encMessages,
  network,
  serverInfo,
  ...
  for (let encMessage of encMessages) {
    let transferMsg = mercury_wasm.decryptTransferMsg(
      encMessage,
      clientAuthKey
    );

    let tx0Outpoint = mercury_wasm.getTx0Outpoint(
      transferMsg.backup_transactions
    );

    const tx0Hex = await getTx0(tx0Outpoint.txid);

    const isTransferSignatureValid = mercury_wasm.verifyTransferSignature(
      newUserPubkey,
      tx0Outpoint,
      transferMsg
    );
    ...
    const isTx0OutputPubkeyValid = mercury_wasm.validateTx0OutputPubkey(
      statechainInfo.enclave_public_key,
      transferMsg,
      tx0Outpoint,
      tx0Hex,
      network
    ...
    let latestBackupTxPaysToUserPubkey =
      mercury_wasm.verifyLatestBackupTxPaysToUserPubkey(
        transferMsg,
        newUserPubkey,
        network
      );
    ...
    let isTx0OutputUnspent = await verifyTx0OutputIsUnspentAndConfirmed(
      coin,
      tx0Outpoint,
      tx0Hex,
      network
    ...
    for (const [index, backupTx] of transferMsg.backup_transactions.entries()) {
      const isSignatureValid = mercury_wasm.verifyTransactionSignature(
        backupTx.tx,
        tx0Hex,
        feeRateTolerance,
      ...
      const isBlindedMusigSchemeValid = mercury_wasm.verifyBlindedMusigScheme(
        backupTx,
        tx0Hex,
        currentStatechainInfo
      );

      if (!isBlindedMusigSchemeValid.result) {
        console.error(
          `Invalid musig scheme, ${isBlindedMusigSchemeValid.result.msg}`
        );
        sigSchemeValidation = false;
        break;
      }
    ...
    const transferReceiverRequestPayload =
      mercury_wasm.createTransferReceiverRequestPayload(
        statechainInfo,
        transferMsg,
        coin
      );

    let serverPublicKeyHex = await sendTransferReceiverRequestPayload(
      transferReceiverRequestPayload
    );

    let newKeyInfo = mercury_wasm.getNewKeyInfo(
      serverPublicKeyHex,
      coin,
...
const getTx0 = async (tx0_txid) => {
  return await electrumRequest({
    method: "blockchain.transaction.get",
    params: [tx0_txid],
  });
};

const verifyTx0OutputIsUnspentAndConfirmed = async (
  coin,
  tx0Outpoint,
  tx0Hex,
  walletNetwork
) => {
  let tx0outputAddress = mercury_wasm.getOutputAddressFromTx0(
    tx0Outpoint,
    tx0Hex,
    walletNetwork
  );

  let reversedHash = await convertAddressToReversedHash({
    address: tx0outputAddress,
    network: walletNetwork,
  });

  let utxo_list = await electrumRequest({
  ...
  for (let unspent of utxo_list) {
    if (
      unspent.tx_hash === tx0Outpoint.txid &&
      unspent.tx_pos === tx0Outpoint.vout
    ) {
      const block_header = await electrumRequest({
