...
const getBlockHeight = async () => {
  ...
  const blockheight = block_header.blockHeight;
...
const checkDeposit = async (coin, walletNetwork, walletName) => {
  ...
  if (coin.status == CoinStatus.INITIALISED) {
    ...
    const backupTx = await deposit.createTx1(
      newCoin,
      walletNetwork,
      utxo_txid,
      utxo_vout
    ...
    const activity = utils.createActivity(
      activity_utxo,
      coin.amount,
      "Deposit"
    );
...
const checkWithdrawal = async (coin, walletNetwork, walletName) => {
  ...
  if (coin.tx_cpfp) {
    if (txid) {
      console.error(
        `Coin ${coin.aggregated_address} has both tx_withdraw and tx_cpfp`
      );
    }
    txid = coin.tx_cpfp;
  }
  ...
  if (!txid) {
    console.error(
      `Coin ${coin.aggregated_address} has neither tx_withdraw nor tx_cpfp`
    );
  }
...
const checkTransfer = async (coin, walletName) => {
  ...
  let isTransferred = await checkTransfer(coin.statechain_id);
...
const updateWallet = async (wallet) => {
  ...
  for (let i = 0; i < wallet.coins.length; i++) {
    ...
    if (
      ...
      let withdrawalResult = await checkWithdrawal(
        coin,
        wallet.network,
        wallet.name
      );
