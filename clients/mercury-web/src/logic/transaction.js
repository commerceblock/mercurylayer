...
const newTransaction = async (
  ...
  const serverPartialSigRequest =
    partialSigRequest.partial_signature_request_payload;

  let serverPartialSig = await signSecond(serverPartialSigRequest);
  ...
  const signature = mercury_wasm.createSignature(
    msg,
    clientPartialSig,
    serverPartialSig,
    session,
  ...
  const signed_tx = mercury_wasm.newBackupTransaction(
    encodedUnsignedTx,
    signature
  );
