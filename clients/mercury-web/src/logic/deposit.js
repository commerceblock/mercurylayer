...
const newAddress = async (wallet, amount, token_id) => {
  console.log(
    "[newAddress]: wallet, amount, token_Id",
    wallet,
    amount,
    token_id
  );
  ...
  const depositMsg1Response = await initPod(depositMsg1);

  const depositInitResult = mercury_wasm.handleDepositMsg1Response(
    coin,
    depositMsg1Response
  );
  ...
  let aggregatedPublicKey = mercury_wasm.createAggregatedAddress(
    coin,
    wallet.network
  );
