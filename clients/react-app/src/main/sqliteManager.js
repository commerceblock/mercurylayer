...
const createTables = async (db) => {
  await run(
    db,
    'CREATE TABLE IF NOT EXISTS wallet (wallet_name TEXT NOT NULL UNIQUE, wallet_json TEXT NOT NULL)',
    []
  )
  await run(
    db,
    'CREATE TABLE IF NOT EXISTS backup_txs (statechain_id TEXT NOT NULL, wallet_name TEXT NOT NULL, txs TEXT NOT NULL)',
    []
  )
}
...
const upsertWallet = async (db, wallet) => {
  await run(
    db,
    'INSERT INTO wallet (wallet_name, wallet_json) VALUES (?, ?) ON CONFLICT(wallet_name) DO UPDATE SET wallet_json = ?',
    [wallet.name, JSON.stringify(wallet), JSON.stringify(wallet)]
  )
}

const upsertEncryptedWallet = async (db, wallet_name, wallet_json) => {
  await run(
    db,
    'INSERT INTO wallet (wallet_name, wallet_json) VALUES (?, ?) ON CONFLICT(wallet_name) DO UPDATE SET wallet_json = ?',
    [wallet_name, wallet_json, wallet_json]
  )
}
...
const getEncryptedWallets = async (db) => {
  return new Promise((resolve, reject) => {
    db.all('SELECT wallet_name, wallet_json FROM wallet', [], (err, rows) => {
      if (err) {
        reject(err)
      } else {
        const wallets = rows.map((row) => {
          return {
            ...
            encrypted_wallet: row.wallet_json
          }
        })
        resolve(wallets)
      }
