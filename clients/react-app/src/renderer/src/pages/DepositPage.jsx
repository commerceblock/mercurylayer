...
const DepositPage = () => {
    ...
    const generateToken = async () => {
      try {
        ...
        if (pending_deposits.length === 0) {
          ...
          let token = await deposit.newRealToken(loggedInWallet.settings)
      ...
      pending_deposits.forEach(async (dep) => {
        ...
        if (!token.confirmed) {
          ...
          try {
            ...
            const response = await deposit.checkToken(token.token_id, loggedInWallet.settings)
            ...
            if (response.confirmed) {
              ...
              dispatch(
                depositActions.updateConfirmedStatus({ depositId: dep.id, confirmedStatus: true })
              )
              ...
              dispatch(
                depositActions.updateTimeExpiry({
                  depositId: dep.id,
  ...
  const onPayButtonClick = async (dep) => {
    ...
    if (!dep.token.confirmed) {
      ...
      await deposit.confirmDebugToken(dep.token.token_id, loggedInWallet.settings)
  ...
  return (
    <div className="w-full relative bg-whitesmoke h-[926px] flex flex-col items-center justify-start gap-[33px] text-left text-sm text-white font-body-small">
      ...
      <div className="self-stretch flex-1 flex flex-row items-center justify-between p-2.5">
        <div className="self-stretch flex-1 relative">
          <div className="absolute h-full w-[27.55%] top-[0%] right-[0%] bottom-[0%] left-[72.45%] flex flex-row items-center justify-center gap-[7px]">
            ...
            <div className="relative font-extralight text-gray-500">BTC Details</div>
          </div>
          <div className="absolute h-full w-[32.01%] top-[0%] right-[30.71%] bottom-[0%] left-[37.28%] flex flex-row items-center justify-center gap-[7px]">
            ...
            <div className="relative font-extralight text-gray-500">Choose Amount</div>
          </div>
          <div className="absolute h-full w-[34.12%] top-[0%] right-[65.88%] bottom-[0%] left-[0%] flex flex-row items-center justify-center gap-[5px]">
            ...
            <div className="relative font-extralight text-gray-500">Pay Fee</div>
          </div>
        </div>
      </div>
...

      {
        ...
        loading ? (
          ...
          pending_deposits.map((deposit, index) => (
            <div
              key={index}
              className="self-stretch h-[480px] overflow-y-auto shrink-0 flex flex-col items-center justify-start p-2.5 box-border"
            >
              <TokenInfoCard
                key={index}
                deposit={deposit}
      ...
      <div className="self-stretch flex-1 overflow-hidden flex flex-col items-end justify-center p-2.5">
        <button
          className={`cursor-pointer [border:none] p-0 w-[90px] rounded-sm shadow-[0px_4px_4px_rgba(0,_0,_0,_0.25)] h-[30px] overflow-hidden shrink-0 flex flex-row items-end justify-end ${
            pending_deposits.some((deposit) => deposit.token.confirmed)
              ? 'bg-mediumslateblue-200'
      ...
      {showNoTokenWindow && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded-md text-black">
            ...
            <button
              className="cursor-pointer shadow-[0px_4px_4px_rgba(0,_0,_0,_0.25)] tracking-[-0.02em] leading-[22px] text-white bg-mediumslateblue-200"
              onClick={onCloseNoTokenWindow}
            >
              OK
            </button>
          </div>
