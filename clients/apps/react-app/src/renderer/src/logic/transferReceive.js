...
const process_encrypted_message = async (coin, encMessages, network, serverInfo, walletName) => {
  ...
  for (let encMessage of encMessages) {
    ...
    let latestBackupTxPaysToUserPubkey = mercury_wasm.verifyLatestBackupTxPaysToUserPubkey(
      transferMsg,
      newUserPubkey,
      network
    )
    ...
    const transferReceiverRequestPayload = mercury_wasm.createTransferReceiverRequestPayload(
      statechainInfo,
      transferMsg,
      coin
    )

    let serverPublicKeyHex = await window.api.sendTransferReceiverRequestPayload(
