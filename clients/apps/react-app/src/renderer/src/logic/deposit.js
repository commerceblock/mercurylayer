...
const checkToken = async (token_id, walletSettings) => {
  let token = await window.api.checkToken(token_id, walletSettings)
  return token
}

const newRealToken = async (walletSettings) => {
  let token = await window.api.getRealToken(walletSettings)
  return token
}

const confirmDebugToken = async (token_id, walletSettings) => {
  console.log('[deposit.js]: going through confirm debug token', token_id)
  let token = await window.api.confirmDebugToken(token_id, walletSettings)
  return token
}
...
const newAddress = async (wallet, amount, token_id) => {
  ...
  let aggregatedPublicKey = mercury_wasm.createAggregatedAddress(coin, wallet.settings.network)
...
const createTx1 = async (coin, wallet_network, tx0_hash, tx0_vout) => {
  if (coin.status !== CoinStatus.INITIALISED) {
    throw new Error(
      `The coin with the aggregated address ${aggregated_address} is not in the INITIALISED state`
    )
  }

  if ('utxo_txid' in coin && 'input_vout' in coin) {
    throw new Error(
      `The coin with the aggregated address ${aggregated_address} has already been deposited`
    )
  }
