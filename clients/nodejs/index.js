...
async function main() {
  ...
  const urlElectrum = "tcp://signet-electrumx.wakiyamap.dev:50001"; //config.get('electrumServer');
  ...
  program
    ...
    .action(async (wallet_name, amount) => {
      const address_info = await deposit.getDepositBitcoinAddress(
        db,
        wallet_name,
        amount
      );
  ...
  program
    .command("broadcast-backup-transaction")
    ...
    .action(async (wallet_name, statechain_id, to_address, options) => {
      ...
      let tx_ids = await broadcast_backup_tx.execute(
        electrumClient,
        db,
        wallet_name,
        statechain_id,
  ...
  program
    ...
    .action(async (wallet_name, statechain_id, to_address, options) => {
      ...
      const txid = await withdraw.execute(
        electrumClient,
        db,
        wallet_name,
        statechain_id,
  ...
  program
    ...
    .action(async (wallet_name, statechain_id, to_address, options) => {
      ...
      let coin = await transfer_send.execute(
        electrumClient,
        db,
        wallet_name,
        statechain_id,
  ...
  program
    .command("transfer-receive")
    .description("Retrieve coins from server")
    .argument("<wallet_name>", "name of the wallet")
    .action(async (wallet_name) => {
      await coin_status.updateCoins(electrumClient, db, wallet_name);

      let received_statechain_ids = await transfer_receive.execute(
        electrumClient,
        db,
        wallet_name
      );
