...
async function main() {
  ...
  program.command('create-wallet')
    ...
    .action(async (name) => {
      ...
      await ml_lib.createWallet(name);
    ...
    .action(async (wallet_name) => {
      ...
      const token = await ml_lib.newToken(wallet_name);
    ...
    program.command('list-tokens')
      ...
      .action(async (wallet_name) => {
        ...
        let wallet = await ml_lib.getWalletTokens(wallet_name);
    ...
    .action(async (wallet_name, amount) => {
      ...
      const address_info = await ml_lib.getDepositBitcoinAddress(wallet_name, amount);
    ...
    program.command('broadcast-backup-transaction')
      ...
      .action(async (wallet_name, statechain_id, to_address, options) => {
       ...
       let tx_ids = await ml_lib.broadcastBackupTransaction(wallet_name, statechain_id, to_address, options);
    ...
    program.command('list-statecoins')
      ...
      .action(async (wallet_name) => {
        ...
        let coins = await ml_lib.listStatecoins(wallet_name);
    ...
    program.command('withdraw')
      ...
      .action(async (wallet_name, statechain_id, to_address, options) => {
        ...
        const txid = await ml_lib.withdrawCoin(wallet_name, statechain_id, to_address, options);
    ...
    program.command('new-transfer-address')
      ...
      .action(async (wallet_name, options) => {
        ...
        let res = await ml_lib.newTransferAddress(wallet_name, options);
    ...
    program.command('transfer-send')
      ...
      .action(async (wallet_name, statechain_id, to_address, options) => {
        ...
        let coin = await ml_lib.transferSend(wallet_name, statechain_id, to_address, options);
    ...
    program.command('transfer-receive')
      ...
      .action(async (wallet_name) => {
        ...
        let received_statechain_ids = await ml_lib.transferReceive(wallet_name);
