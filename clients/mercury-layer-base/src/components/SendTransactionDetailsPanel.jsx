...
const SendTransactionDetailsPanel = ({ wallet, selectedCoin }) => {
    ...
    const onSendClick = async () => {
        ...
        if (toAddress === '') {
            alert('enter a statecoin address to send the coin to.');
            return;
        }
        ...
        let transferData = await transferSend.execute(wallet, selectedCoin, backupTxs, toAddress);
                    ...
                    <>
                        <div className="self-stretch overflow-hidden flex flex-row items-center justify-center py-5 px-2.5 text-center text-sm text-primary">
                            <div className="w-[345px] flex flex-col items-center justify-center gap-[7px]">
                                ...
                                <div className="self-stretch rounded-md bg-white box-border h-12 flex flex-row items-start justify-start p-3 text-left text-xs border-[1px] border-solid border-primary">
                                    <input className="flex-1 relative tracking-[-0.02em] leading-[22px] focus:outline-none" placeholder="Send to statecoin address" type="text" value={toAddress} onChange={(e) => setToAddress(e.target.value)} />
                                </div>
                            </div>
                        </div>
                        <div className="self-stretch overflow-hidden flex flex-row items-center justify-center p-2.5">
                            <button className="cursor-pointer rounded-sm bg-royalblue-200 
                                ...
                                onClick={onSendClick}>
                                <div className="relative text-white">Send Statecoin</div>
                            </button>
                        </div>
                    </>
                ) : (
                    <div className="self-stretch overflow-hidden flex flex-row items-center justify-center py-5 px-2.5 text-center text-sm text-primary">
                        <div className="relative">Please select a coin to send first</div>
                    </div>
                )
            )}
        </div>
...
export default SendTransactionDetailsPanel;