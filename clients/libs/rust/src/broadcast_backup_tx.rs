...
use mercurylib::wallet::{coin_status::CoinStatus, cpfp_tx};
...
pub async fn execute(client_config: &ClientConfig, wallet_name: &str, statechain_id: &str, to_address: &str, fee_rate: Option<f64>) -> Result<()> {
    ...
    if coin.status() != CoinStatus::Confirmed && coin.status() != CoinStatus::InTransfer {
    ...
    coin.set_tx_cpfp(Some(txid.to_string()));
    coin.set_withdrawal_address(Some(to_address.to_string()));
    coin.set_status(CoinStatus::Withdrawing);

    let signed_statechain_id = coin.signed_statechain_id().unwrap().to_string();
