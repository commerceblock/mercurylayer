...
async fn ta02(client_config: &ClientConfig, wallet1: &Wallet, wallet2: &Wallet)  -> Result<()> {
    ...
    let result = mercuryrustlib::transfer_sender::execute(&client_config, &wallet2_transfer_adress, &wallet1.name, statechain_id, None).await;

    assert!(result.is_err());

    let error_msg = result.err().unwrap().to_string();

    assert!(error_msg == "Coin is duplicated. If you want to proceed, use the command '--force, -f' option. This will cause PERMANENT LOSS of duplicated coin funds.");
...
pub async fn execute() -> Result<()> {
    ...
    ta02(&client_config, &wallet1, &wallet2).await?;
