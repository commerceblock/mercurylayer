...
pub async fn tb04(client_config: &ClientConfig, wallet1: &Wallet, wallet2: &Wallet) -> Result<()> {
    ...
    mercuryrustlib::transfer_sender::execute(&client_config, &wallet2_transfer_adress, &wallet1.name, statechain_id, None, force_send, Some(batch_id.clone())).await?;
    
    // receiver confirms that payment hash was generated by the server
    let response = mercuryrustlib::lightning_latch::get_payment_hash(&client_config, &batch_id).await?;

    assert!(response.is_some());
    assert!(response.unwrap() == hash);
