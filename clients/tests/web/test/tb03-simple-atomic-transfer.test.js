...
describe('TB03 - Atomic swap with first party steal', () => {
    test("expected flow", async () => {
        ...
        const toAddress3_for_steal = await mercuryweblib.newTransferAddress(wallet3.name, true);
        ...
        let transferReceive3 = undefined;
        try {
            transferReceive3 = await mercuryweblib.transferReceive(clientConfig, wallet3.name);
        } catch (error) {
            // Assert the captured error message
...
describe('TB03 - Atomic swap with second party steal', () => {
    test("expected flow", async () => {
