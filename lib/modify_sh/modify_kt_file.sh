...
FIND_TEXT_TRANSFER_RECEIVER_POST_RESPONSE_PAYLOAD="data class TransferReceiverPostResponsePayload("
FIND_TEXT_TRANSFER_RECEIVER_GET_RESPONSE_PAYLOAD="data class TransferReceiverGetResponsePayload("
...
find "$SEARCH_DIR" -type f -name "$FILE_PATTERN" -exec sed -i \
    ...
    -e "/$FIND_TEXT_TRANSFER_RECEIVER_POST_RESPONSE_PAYLOAD/i $ADD_TEXT" \
    -e "/$FIND_TEXT_TRANSFER_RECEIVER_GET_RESPONSE_PAYLOAD/i $ADD_TEXT" \
