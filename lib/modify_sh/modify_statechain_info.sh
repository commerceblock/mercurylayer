...
add_serial_name() {
    ...
    /data class StatechainInfo\(/ { in_data_class = 1 }
    ...
    in_data_class && /var `serverPubnonce`:/ {
        print "\t@SerialName(\"server_pubnonce\")"
    }
    in_data_class && /var `txN`:/ {
        print "\t@SerialName(\"tx_n\")"
    }
...
echo "@SerialName annotations added to StatechainInfo successfully."
