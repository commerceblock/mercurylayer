...
add_serial_name() {
    ...
    /data class TransferUnlockRequestPayload\(/ { in_data_class = 1 }
    ...
    in_data_class && /var `authPubKey`:/ {
        print "\t@SerialName(\"auth_pub_key\")"
    }
...
echo "@SerialName annotations added to TransferUnlockRequestPayload successfully."
