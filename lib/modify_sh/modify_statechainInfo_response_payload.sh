...
add_serial_name() {
    ...
    /data class StatechainInfoResponsePayload\(/ { in_data_class = 1 }
    /^}/ { if (in_data_class) in_data_class = 0 }

    in_data_class && /var `enclavePublicKey`:/ {
        print "\t@SerialName(\"enclave_public_key\")"
    }
    in_data_class && /var `numSigs`:/ {
        print "\t@SerialName(\"num_sigs\")"
    }
    in_data_class && /var `statechainInfo`:/ {
        print "\t@SerialName(\"statechain_info\")"
    }
    in_data_class && /var `x1Pub`:/ {
        print "\t@SerialName(\"x1_pub\")"
    }
...
echo "@SerialName annotations added to StatechainInfoResponsePayload successfully."
