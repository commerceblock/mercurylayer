...
add_serial_name() {
    ...
    /data class TransferSenderRequestPayload\(/ { in_data_class = 1 }
    ...
    in_data_class && /var `batchId`:/ {
        print "\t@SerialName(\"batch_id\")"
    }
...
echo "@SerialName annotations added to TransferSenderRequestPayload successfully."
