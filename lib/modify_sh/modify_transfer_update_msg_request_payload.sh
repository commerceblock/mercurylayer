...
add_serial_name() {
    ...
    /data class TransferUpdateMsgRequestPayload\(/ { in_data_class = 1 }
    ...
    in_data_class && /var `newUserAuthKey`:/ {
        print "\t@SerialName(\"new_user_auth_key\")"
    }
    in_data_class && /var `encTransferMsg`:/ {
        print "\t@SerialName(\"enc_transfer_msg\")"
    }
...
echo "@SerialName annotations added to TransferUpdateMsgRequestPayload successfully."
